...
do
    ...
    then
        srun -p gpu -t 10:00:00 --mem-per-cpu 1 --gres=gpu:1 -J caps_${data}_l2 -o caps_${data}_l2.out -e caps_${data}_l2.err python train.py --model caps --data ${data} --epochs 15 --batch-size 100 --regularizer l2 --regularizer-weight 0.001 --name caps_${data}_15_l2 --save-freq 1 --tensorboard &
    elif [ $model = "l21" ]
    then
        srun -p gpu -t 10:00:00 --mem-per-cpu 1 --gres=gpu:1 -J caps_${data}_l21 -o caps_${data}_l21.out -e caps_${data}_l21.err python train.py --model caps --data ${data} --epochs 15 --batch-size 100 --regularizer l21 --regularizer-weight 0.001 --name caps_${data}_15_l21 --save-freq 1 --tensorboard &
    elif [ $model = "op" ]
    then
        srun -p gpu -t 10:00:00 --mem-per-cpu 1 --gres=gpu:1 -J caps_${data}_op -o caps_${data}_op.out -e caps_${data}_op.err python train.py --model caps --data ${data} --epochs 15 --batch-size 100 --regularizer operator_norm --regularizer-weight 0.001 --name caps_${data}_15_op --save-freq 1 --tensorboard &
    elif [ $model = "conv" ]
    then
