...
do
    ...
    then
        srun -p gpu -t 10:00:00 --mem-per-cpu 1 --gres=gpu:1 -J convcaps_mnist -o convcaps_mnist.out -e convcaps_mnist.err python train.py --model convcaps --data mnist --epochs 15 --batch-size 100 --name convcaps_mnist_15 --save-freq 1 --tensorboard &
    elif [ model = "full" ]
    then
        srun -p gpu -t 10:00:00 --mem-per-cpu 1 --gres=gpu:1 -J fullcaps_mnist -o fullcaps_mnist.out -e fullcaps_mnist.err python train.py --model fullcaps --data mnist --epochs 15 --batch-size 100 --name fullcaps_mnist_15 --save-freq 1 --tensorboard &
    else
