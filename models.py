...
class BaseModel:
    def __init__(self, name, n_class, image_shape, loss, tensorboard, filename=None):
        self.name = name
...
class CapsNet(BaseModel):
    def _new_model(self): #ADD ROUTINGS param: routings
        """
        ...
        """
        x = layers.Input(shape=self.image_shape)
        ...
        decoder.add(layers.Dense(512, activation='relu', input_dim=16*self.n_class))
        decoder.add(layers.Dense(1024, activation='relu'))
        decoder.add(layers.Dense(np.prod(self.image_shape), activation='sigmoid'))
        decoder.add(layers.Reshape(target_shape=self.image_shape, name='out_recon'))
        ...
        # manipulate model
        noise = layers.Input(shape=(self.n_class, 16))
